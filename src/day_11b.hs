...
findSimultaneousFlash :: EnergyLevels -> Int
findSimultaneousFlash energyLevels =
    let nrCells = gridSize * gridSize
        go :: EnergyLevels -> Int -> Int
        go levels nrSteps =
            let nrSteps' = nrSteps + 1
                (levels', c) = singleStep levels
            in if c == nrCells then nrSteps' else go levels' nrSteps'
    in go energyLevels 0
...
main = do
    ...
    print $ findSimultaneousFlash energyLevels
