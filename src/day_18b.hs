...
instance Eq SnailFishNumber where
    Literal n == Literal m = n == m
    Pair n m == Pair n' m' = n == n' && m == m'
    Literal _ == Pair _ _ = False
    Pair _ _ == Literal _ = False
...
largestSum :: [SnailFishNumber] -> Int
largestSum numbers =
    let pairs = [(n, m) | n <- numbers, m <- numbers, n /= m]
    in foldl (\acc (a, b) -> let m = magnitude $ reduce $ Pair a b in if m > acc then m else acc) 0 pairs
